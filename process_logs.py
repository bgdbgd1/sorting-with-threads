...
def process_logs(nr_files, file_size, intervals, pipeline, nr_parallel_threads):
    ...
    log_files = glob.glob(f'logs_determine_bandwidth/logs_nr_files_{nr_files}_file_size_{file_size}_intervals_{intervals}_50_iterations_{nr_parallel_threads}_threads_with_rules_50MB_with_distributed_minio/*.log')
            ...
            for line in log_file.readlines():
                ...
                for task_name, phrase_tasks in phrases_stage_2.items():
                    for phrase in phrase_tasks:
                        if phrase in line:
                            format_line(stage_name, phrase, line, task_name)
    file_path = f'logs_determine_bandwidth/logs_nr_files_{nr_files}_file_size_{file_size}_intervals_{intervals}_50_iterations_{nr_parallel_threads}_threads_with_rules_50MB_with_distributed_minio/results_nr_files_{nr_files}_file_size_{file_size}_intervals_{intervals}_{pipeline}.json'
