...
def sort_category(
    ...
    partitions_length,
    experiment_number,
    scheduled_files_statuses_sort_write
):
    try:
        if (
        ...
        scheduled_files_statuses_sort_write[partition_name] = 'NOT_SORTED'
...
def write_category(
    ...
    try:
        ...
        scheduled_files_statuses_sort_write[category_name] = 'SORTED'
...
def execute_stage_2_pipeline(
    ...
    with mp.Manager() as manager:
        ...
        scheduled_files_statuses_sort_write = manager.dict()
        ...
        while len(files_written) < len(partitions_names):
            ...
            for part_name in partitions_names:
                if (
                        ...
                        files_sorted.get(part_name) is None and
                        scheduled_files_statuses_sort_write[part_name] == 'NOT_SORTED'
                ):
                    scheduled_files_statuses_sort_write[part_name] = 'SORTED'
                ...
                if (
                        files_sorted.get(part_name) is not None and
                        files_written.get(part_name) is None and
                        scheduled_files_statuses_sort_write[part_name] == 'SORTED'
                ):
                    scheduled_files_statuses_sort_write[part_name] = 'WRITING'
