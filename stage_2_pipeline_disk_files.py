...
def execute_stage_2_pipeline(
    ...
    queue_read = mp.Queue()
    pool_read = mp.Pool(nr_reading_processes, read_worker, (queue_read, minio_ip, intermediate_bucket, experiment_number))
    counter_jobs = 0
    for partition_name in partitions:
        if not os.path.exists(f'{PREFIX}stage_2/server_{SERVER_NUMBER}/read/{partition_name}'):
            os.mkdir(f'{PREFIX}stage_2/server_{SERVER_NUMBER}/read/{partition_name}')
        if not os.path.isdir(f'{PREFIX}stage_2/server_{SERVER_NUMBER}/read_finished/{partition_name}'):
            os.mkdir(f'{PREFIX}stage_2/server_{SERVER_NUMBER}/read_finished/{partition_name}')
    for partition_name, partition_data in partitions.items():
        for file_data in partition_data:
            ...
            queue_read.put({
                ...
                "start_index": file_data['start_index'],
                "end_index": file_data['end_index'],
            })
            # read_partition(
            #     partition_name,
            #     file_data['file_name'],
